...
TEST_F(AsyncATHandlerHTTPTest, HttpSocketOpenWithLongTimeout) {
  bool testResult = runInFreeRTOSTask(
      [this]() {
        ...
        testStream->mockResponseWithDelay("+QIOPEN: 0,0\r\n", 2000);
...
TEST_F(AsyncATHandlerHTTPTest, SendHttpDataOverTcp) {
  bool testResult = runInFreeRTOSTask(
      [this]() {
        ...
        static auto promptResponderTask = [](void* pvParameters) {
          SerialCommunicator* localTestStream = static_cast<SerialCommunicator*>(pvParameters);
          vTaskDelay(pdMS_TO_TICKS(100));
          localTestStream->mockResponse(">\r\n");
          vTaskDelete(nullptr);
        };
        TaskHandle_t promptTaskHandle = nullptr;
        xTaskCreate(
            promptResponderTask, "PromptResponder", configMINIMAL_STACK_SIZE * 2, testStream, 1,
            &promptTaskHandle);
        ...
        static auto dataResponderTask = [](void* pvParameters) {
          SerialCommunicator* localTestStream = static_cast<SerialCommunicator*>(pvParameters);
          vTaskDelay(pdMS_TO_TICKS(100));
          localTestStream->mockResponse("OK\r\n");
          localTestStream->mockResponse("SEND OK\r\n");
          vTaskDelete(nullptr);
        };
        TaskHandle_t dataTaskHandle = nullptr;
        xTaskCreate(
            dataResponderTask, "DataResponder", configMINIMAL_STACK_SIZE * 2, testStream, 1,
            &dataTaskHandle);

        SerialCommunicator* stream = static_cast<SerialCommunicator*>(handler->getStream());
        testStream->write(reinterpret_cast<const uint8_t*>(httpRequest.c_str()), dataLength);
        testStream->flush();
...
TEST_F(AsyncATHandlerHTTPTest, SendHttpDataChunked) {
  bool testResult = runInFreeRTOSTask(
      [this]() {
        ...
        static auto chunk1Responder = [](void* pvParameters) {
          SerialCommunicator* localTestStream = static_cast<SerialCommunicator*>(pvParameters);
          vTaskDelay(pdMS_TO_TICKS(100));
          localTestStream->mockResponse(">\r\n");
          vTaskDelete(nullptr);
        ...
        xTaskCreate(
            chunk1Responder, "C1Responder", configMINIMAL_STACK_SIZE * 2, testStream, 1,
            &c1ResponderHandle);
        ...
        static auto chunk1DataResponder = [](void* pvParameters) {
          SerialCommunicator* localTestStream = static_cast<SerialCommunicator*>(pvParameters);
          vTaskDelay(pdMS_TO_TICKS(100));
          localTestStream->mockResponse("OK\r\nSEND OK\r\n");
          vTaskDelete(nullptr);
        ...
        xTaskCreate(
            chunk1DataResponder, "C1Data", configMINIMAL_STACK_SIZE * 2, testStream, 1,
            &c1DataHandle);
        ...
        static auto chunk2Responder = [](void* pvParameters) {
          SerialCommunicator* localTestStream = static_cast<SerialCommunicator*>(pvParameters);
          vTaskDelay(pdMS_TO_TICKS(100));
          localTestStream->mockResponse(">\r\n");
          vTaskDelete(nullptr);
        ...
        xTaskCreate(
            chunk2Responder, "C2Responder", configMINIMAL_STACK_SIZE * 2, testStream, 1,
            &c2ResponderHandle);
        ...
        static auto chunk2DataResponder = [](void* pvParameters) {
          SerialCommunicator* localTestStream = static_cast<SerialCommunicator*>(pvParameters);
          vTaskDelay(pdMS_TO_TICKS(100));
          localTestStream->mockResponse("OK\r\n");
          localTestStream->mockResponse("SEND OK\r\n");
        ...
        xTaskCreate(
            chunk2DataResponder, "C2Data", configMINIMAL_STACK_SIZE * 2, testStream, 1,
            &c2DataHandle);
