...
SerialCommunicator::SerialCommunicator( ) {
    ...
    mockStream->SetupDefaults();
        ...
        hardwareStream->begin(115200, SERIAL_8N1, 23, 19);
...
void SerialCommunicator::mockResponse(const std::string& data) {
...
std::string SerialCommunicator::GetSentData() {
    return mockStream->GetTxData();
}
...
int SerialCommunicator::peek() {
    return activeStream->peek();
}

void SerialCommunicator::flush() {
    activeStream->flush();
}
