...
TEST_F(AsyncATHandlerWaitResponseTest, WaitResponseWithTimeoutOnly) {
  bool testResult = runInFreeRTOSTask([this]() {
    ...
    // Inject some unsolicited data
    mockStream->InjectRxData("+CREG: 0,1\r\n");
    mockStream->InjectRxData("+CSQ: 15,99\r\n");

    // Give reader task time to process the injected data
    vTaskDelay(pdMS_TO_TICKS(100));

    // Wait for any response with timeout
    int8_t result = handler->waitResponse(1000);  // 1 second timeout

    if (result != 1) {
      throw std::runtime_error("Should have found some response");
    }

    log_i("[Test] Found response as expected");
  }, "TimeoutOnlyTest");
...
TEST_F(AsyncATHandlerWaitResponseTest, WaitResponseWithDefaultTimeout) {
  bool testResult = runInFreeRTOSTask([this]() {
    ...
    // Wait for any response with default timeout
...
TEST_F(AsyncATHandlerWaitResponseTest, WaitResponseTimeout) {
  bool testResult = runInFreeRTOSTask([this]() {
    ...
    int8_t result = handler->waitResponse(200);  // Short timeout

    if (result != -1) {
      throw std::runtime_error("Should have timed out");
    }
