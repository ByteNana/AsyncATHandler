...
TEST_F(AsyncATHandlerAdvancedTest, SimpleSyncCommand) {
  bool testResult = runInFreeRTOSTask(
      [this]() {
        ...
        String response;
        bool success = handler->sendSync("AT", response, 2000);
...
TEST_F(AsyncATHandlerAdvancedTest, VariadicSendCommandHelper) {
  bool testResult = runInFreeRTOSTask(
      [this]() {
        ...
        log_d("[Test] Testing variadic template: sendCommand(\"AT\")");

        ATPromise* promise = handler->sendCommand("AT", "");
        if (!promise) {
...
NATIVE_ONLY(TEST_F(AsyncATHandlerAdvancedTest, UnsolicitedResponseHandling) {
  bool testResult = runInFreeRTOSTask(
      [this]() {
        if (!handler->begin(*testStream)) throw std::runtime_error("Handler begin failed");
